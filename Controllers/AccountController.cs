...
namespace CompShop2.Controllers
{
    ...
    {
        public CSEntities db = new CSEntities();
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    // это для того что бы по дефолту была роль
                    await UserManager.AddToRoleAsync(user.Id, "seller");
                    await SignInManager.SignInAsync(user, isPersistent:false, rememberBrowser:false);
                   //это для связывания логина с БД
                    var identity = (ClaimsPrincipal)Thread.CurrentPrincipal;
                    var email = user.Email;
                    //var rolle = user.Roles;
                    Workers p1 = new Workers { Name = email, Role = 2 };
                    db.Workers.Add(p1);
                    db.SaveChanges();
